...
class LabelBuilder(val kakaoMap: KakaoMap) {
        ...
        val options = LabelOptions.from(position)
            .setStyles(
                kakaoMap.labelManager?.addLabelStyles(
                    LabelStyles.from(
                        LabelStyle.from(R.drawable.aim_icon2).setZoomLevel(10),
                        LabelStyle.from(R.drawable.aim_icon4).setZoomLevel(18),
                    )
                )
            )
    ...
    fun makeToiletLabel(toiletModel: ToiletModel): Label? {
        ...
        val styles = if (isLiked == true) {
            kakaoMap.labelManager?.addLabelStyles(
                LabelStyles.from(
                    LabelStyle.from(R.drawable.saved_pin1).setZoomLevel(15),
                    LabelStyle.from(R.drawable.saved_pin2).setZoomLevel(16),
                    LabelStyle.from(R.drawable.saved_pin3).setZoomLevel(17),
                    LabelStyle.from(R.drawable.saved_pin4).setZoomLevel(18)
                )
            )
        } else {
            kakaoMap.labelManager?.addLabelStyles(
